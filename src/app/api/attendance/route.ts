...
export async function GET(req: Request) {
  try {
    ...
    // Users who have "accommodation" marked as yes (case-insensitive)
    // AND have attended at least one session for the selected year
    if (type === "accommodation") {
      const attendedIds = await Attendance.find({ year }).distinct("user");

      const accommodationUsers = await User.find({
        _id: { $in: attendedIds },
        accommodation: { $regex: /^yes$/i }, // matches yes, Yes, YES
      });
